...
async function getCategories(): Promise<string[]> {
  const { data, error } = await supabase.from("artwork_categories").select("name").order("name");
  if (error) return ["All"];
...
export default async function ArtworksPage() {
  const [categories, artworks, fields] = await Promise.all([
    getCategories(),
    getArtworks(),
  ...
  return (
    <main className="w-[min(1200px,92%)] mx-auto flex flex-col gap-10 py-10 md:py-16">
      <FadeInSection className="flex flex-col gap-5">
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-zinc-500 mb-2">Gallery</p>
