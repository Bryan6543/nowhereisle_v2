...
export default async function Home() {
  const [blogs, fields] = await Promise.all([getBlogs(), getPageFields("dashboard")]);
    ...
    <main className="w-[min(1200px,92%)] mx-auto flex flex-col gap-14 py-10 md:py-16">
      <FadeInSection className="flex flex-col gap-5">
        <p className="text-sm uppercase tracking-[0.2em] text-zinc-500">
          {t("kicker", "Studio World")}
        </p>
        <h1 className="dashboard_head">{t("title", "A World in the Making")}</h1>
        <HeroSlider />
      </FadeInSection>

      <FadeInSection className="flex flex-col gap-4 max-w-3xl">
        <h2 className="dashboard_head">{t("section_title", "Into the Lost Holy Capital")}</h2>
        <p className="lora_body text-zinc-300">
          {t(
            "section_text",
            "Kradel Tactics is the whole reason the studio exists. Check out our art gallery and the lore posts while the project is still under development.",
          )}
        </p>
        <Image
      ...
      <FadeInSection className="flex flex-col gap-6">
        <div className="flex items-end justify-between gap-4">
          <h2 className="dashboard_head">{t("lore_title", "Latest Lore Wisdom")}</h2>
          <Link
